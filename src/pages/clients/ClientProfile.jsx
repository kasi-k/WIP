...
import Client from "../../assets/client_avatar.png"
...
const ClientProfile = () => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("newClientsData");
    let allClients = [...ClientTableData];
    let newClients = [];
    if (saved) {
      try {
    ...
    // Also check if we deleted this client just in case
    const deleted = localStorage.getItem("deletedClients");
    ...
    // Since we now store edits in newClientsData even for default clients,
    // newClientsData takes precedence if the ID matches.
    const foundNew = newClients.find((c) => c.clientID === id);
    if (foundNew) {
      setClient(foundNew);
    } else {
      const foundDefault = ClientTableData.find((c) => c.clientID === id);
      setClient(foundDefault);
    }
  }, [id]);
  ...
  return (
    <div className="bg-[#f4f4f4] p-6 font-sans overflow-y-scroll h-full">
      {/* Header */}
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full items-start">
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6 min-w-0">
          ...
          <div className="bg-white rounded-[20px] p-8 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex flex-col items-center text-center">
            ...
            <button className="w-full py-3 bg-white border-[1.5px] border-[#e2e8f0] hover:border-[#001552] hover:text-[#001552] text-[#334155] rounded-[14px] text-[14px] font-bold flex items-center justify-center gap-2.5 transition-all shadow-sm">
          ...
          <div className="bg-white rounded-[20px] p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex flex-col gap-4">
            <div className="flex items-center gap-4 p-3 border border-[#f1f5f9] bg-[#f8fafc] rounded-[14px]">
          ...
          <div className="bg-white rounded-[20px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex flex-col flex-1">
            ...
            <div className="p-8">
              <div className="mb-6 flex">
                <button className="bg-white text-[13px] font-bold text-[#334155] px-4 py-2 rounded-xl border border-[#e2e8f0] flex items-center gap-2 hover:bg-gray-50 transition-colors shadow-sm">
                  All Time (5)
              ...
              <div className="flex flex-col gap-4">
                {appointments.map((apt, idx) => (
                  <div key={idx} className="flex items-center justify-between group bg-white border border-[#f1f5f9] rounded-[16px] p-4 hover:border-blue-100 hover:bg-[#f8fafc] transition-all shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)]">
                    <div className="flex items-center gap-5">
                      <div className="text-center w-14 h-14 bg-white rounded-xl flex flex-col justify-center items-center border border-[#e2e8f0] shadow-sm">
